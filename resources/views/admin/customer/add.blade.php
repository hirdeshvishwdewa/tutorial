		...
		<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">
			<div class="box box-primary">
				...
				<div class="row">
					...
					<div class="col-md-8">
						<form action="{{ route('customer-create') }}" method="POST" novalidate="">
							@csrf
							<div class="box-body">
								...
								<div class="form-group">
									...
									<input type="text" class="form-control" id="name" name="name" max="256" min="3" placeholder="{{ __('admin.customer.name_placeholder') }}" value="{{ old('name') ?? '' }}" autofocus="true">
								...
								<div class="form-group">
									...
									<input type="text" class="form-control" id="email" name="email" max="255" min="8" placeholder="{{ __('admin.customer.email_placeholder') }}" value="{{ old('email') ?? '' }}">
									<p class="error-message text-red">
										@if($errors->has('email'))
											{{ $errors->first('email') }}
										@endif
								...
								<div class="form-group">
									...
									<input type="text" class="form-control" id="phone" name="phone" max="10" min="10" placeholder="{{ __('admin.customer.phone_placeholder') }}" value="{{ old('phone') ?? '' }}">
									<p class="error-message text-red">
										@if($errors->has('phone'))
											{{ $errors->first('phone') }}
										@endif
								...
								<div class="form-group">
									...
									<select class="form-control" id="gender" name="gender">
										...
										<option value="others" {{ old('gender') && old('gender') == 'others' ? 'selected' : '' }}>Others</option>
									</select>
									<p class="error-message text-red">
									...
									<select class="form-control" id="class" name="class">
										...
										@if(count($classes))
											@foreach($classes as $key=>$class)
												<option value="{{ $key }}" {{ old('class') && old('class') == $key ? 'selected' : '' }}>{{ $class }}</option>
											@endforeach
										@endif
									</select>
									<p class="error-message text-red">
								...
								<div class="form-group">
									...
									<input type="text" class="form-control" id="school" name="school" maxlength="256" minlength="3" placeholder="{{ __('admin.customer.school_placeholder') }}" value="{{ old('school') ?? '' }}">
									<p class="error-message text-red">
								...
								<div class="form-group">
									...
									<select class="form-control form-select" id="status" name="status">
										<option value="" {{ old('status') === '' ? 'selected' : '' }}>{{ __('admin.customer.select_status') }}</option>
										<option value="1" {{ old('status') === 1 ? 'selected' : '' }}>{{ __('admin.customer.status_enable') }}</option>
										<option value="0" {{ old('status') === 0 ? 'selected' : '' }}>{{ __('admin.customer.status_disable') }}</option>
									</select>
